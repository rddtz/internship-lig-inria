...
#+TITLE: Integration and Benchmarking of Parallel Writing for Deisa-Ray
#+AUTHOR: Rayan Raddatz de Matos



This report details the progress, challenges, and solutions developed
to enable efficient parallel writing of distributed Dask arrays within
the =deisa-ray= framework. The primary focus was overcoming the native
limitations of dask library when interacting with parallel file
systems.

#+begin_quote
[!NOTE]
The file [[file:journal.org][journal.org]] has some miscellaneous of notes about
what was being done, the progress, the scripts crated and ideas during
the time I worked on this project. One may want to take a look or use
an AI to extract the thought process for some conclusions made here.
#+end_quote

* Exploration Phase

The project started at [[https://github.com/deisa-project/deisa-ray/tree/dc115a8648aaee70636a096c61ccf19767771cd4][deisa-ray commit dc115a8]], the first steps
consisted in get used to the project technologies such as Dask and
Ray. Get familiar with [[https://www.grid5000.fr/w/Grid5000:Home][Grid5000]] was also required for experiments
...
the try to integrating it with Dask through [[https://docs.h5py.org/en/stable/index.html][h5py]] HDF5 python
interface.

Dask has saving functions capable of writing to HDF5 as detailed in
its [[https://docs.dask.org/en/latest/generated/dask.array.to_hdf5.html][documentation]]. However, this functions are not compatible with
...
dask community was to use [[https://zarr.readthedocs.io/en/stable/][Zarr]] instead of HDF5.
...
A [[https://github.com/deisa-project/deisa-ray/pull/67#issue-3886559798][Pull Request was then opened]] to deisa-ray with initial
...
cluster of the [[https://www.grid5000.fr/w/Rennes:Hardware][Grid'5000 Rennes]] site. We also varied the number of MPI
...
[[https://github.com/dask/dask/pull/12309][pull request was opened]] in the dask GitHub repository trying to
integrate the approach used for deisa for dask as well.


# The proposed approach showed a good scalability, being successfully
# tested at scale. processes per node. Under this high parallelism, the VDS strategy
# showed good results, while maintained highly stable and predictable
# write times.
